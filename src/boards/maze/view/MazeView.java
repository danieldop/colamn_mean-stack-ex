...
public class MazeView extends ViewAbs {
	...
	private boolean isUp, isLeft, isDown, isRight;// isUp=>is arrow up key is
													// down., etc...

	boolean isMouseDown = false;// for board mouse event if user clicked on the
								// mouse.

	/**
	 * @author omerpr @ init the canvas board. @ init the canvas board events.
	 */
	...
	/**
	 * @author omerpr @ enables mouse being dragged on the board. @
	 * 
	 *         NOTICE: board.getData("mousePoint") takes the mouse coordinates
	 *         on the board from the board Object. This structure is used
	 *         inorder to keep the modularisation intact.
	 */
	@Override
	protected void initBoardMouseEvents() {
		board.addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				mouse = (Point) board.getData("mousePoint");// extracting the
															// mouse current
															// location.
				if (mouse != null) {
					squareWidth = (board.getBounds().width - 5)
							/ board.getBoardData()[0].length;
					squareHeight = (board.getBounds().height - 5)
							/ board.getBoardData().length;
					if (e.x >= mouse.x && e.x < mouse.x + squareWidth
							&& e.y >= mouse.y && e.y < mouse.y + squareHeight) {
						isMouseDown = true;
		...
		board.addMouseMoveListener(new MouseMoveListener() {
			Point mouse;
			int squareWidth, squareHeight;// xSquares=>how many squares from
											// mouse x,ySquares=>how many
											// squares from mouse y.
			int mouseX, mouseY;

			/**
			 * 
			 * @param double num1
			 * @param double num2 detects the closest move the user is trying to
			 *        move to and preforms the movement action.
			 */
			...
			public void mouseMove(MouseEvent e) {
				if (isMouseDown) {
					...
					if (mouse != null) {
						...
						preformClosestMove(((double) e.x - mouseX)
								/ (squareWidth), ((double) e.y - mouseY)
								/ (squareHeight));
					}
	...
	/**
	 * setting up all key movement responds. @ ARROW_UP => UP @ ARROW_DOWN =>
	 * DOWN @ ETC . .
	 */
	@Override
		...
		this.board.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (timer == null)// a movement has not been started yet.
				{
					counter = 0;
					timer = new Timer();
					timer.schedule(getMovementTask(), 30);// give 30
															// milliseconds to
															// include diagonal
															// move.
				} else// a movement is already scheduled.
				{
					if (counter > 8)
						counter = 0;
					timer.schedule(getMovementTask(), 100 + (++counter) * 70);// schedule
																				// the
																				// next
																				// move
																				// to
	...
	public void showWonGame() {
		display.syncExec(new Runnable() {
			...
			@Override
			public void run() {
				openMessageBox("You Won!", "Congratulation! you have found the cheese!",
						SWT.OK);
			}
		});
	}
